...
vector<vector<int>> fk(vector<int> leftNible, vector<int> rightNible, vector<int> IP, vector<int> EP, vector<int> key1,
                       vector<vector<int>> s0, vector<vector<int>> s1, vector<int> p4)
{
    ...
    vector<int> afterXOR = performXOR(expanded, key1);
    // cout << endl;
...
vector<vector<int>> encryption(vector<int> p, vector<int> IP, vector<int> IPI, vector<int> EP, vector<int> key1, vector<int> key2,
                               vector<vector<int>> s0, vector<vector<int>> s1, vector<int> p4)
{
    ...
    vector<vector<int>> phisher = fk(leftNible, rightNible, IP, EP, key1, s0, s1, p4);

    vector<int> left = phisher[1];
    vector<int> right = phisher[0];

    vector<vector<int>> answer = fk(left, right, IP, EP, key2, s0, s1, p4);
...
int main()
{
    ...
    // vector<int> p = {0, 1, 0, 0, 0, 0, 0, 1};
    vector<int> p = {1, 1, 0, 1, 0, 1, 0, 1};
    ...
    cout << "Plain text before encryption: ";
    for (auto it : p)
    {
        cout << it << " ";
    }
    ...
    vector<vector<int>> result = encryption(p, IP, IPI, EP, key1, key2, s0, s1, p4);
    vector<int> cipher;
    for (auto it : result)
    {
        for (auto m : it)
    ...
    decryption(cipher, IP, IPI, EP, key1, key2, s0, s1, p4);
